...
public class SmallRedwoodTrunkPlacer extends TrunkPlacer {
  ...
  protected TrunkPlacerType<?> type() {
    return PioneerTreePlacers.Trunk.SMALL_REDWOOD_TRUNK_PLACER.get();
  }
  ...
  public static void placeBase(LevelSimulatedReader world, BiConsumer<BlockPos, BlockState> placer, RandomSource rand, BlockPos.MutableBlockPos pos, TreeConfiguration config, BlockPos start, int heightMin, int heightMax, int offX, int offZ) {
    ...
    int offY = 1;
    while(offY > -3 && world.isStateAtPosition(pos.setWithOffset(start, offX, offY, offZ), BlockState::canBeReplaced)) {
      offY--;
    }
    ...
    for(int i = offY; i < height; i++) {
      ...
      if(i < height - 1)
        ...
        placer.accept(temp, PioneerBlocks.REDWOOD.wood().get().defaultBlockState());
    }
  }
}