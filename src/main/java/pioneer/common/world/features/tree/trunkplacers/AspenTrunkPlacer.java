package pioneer.common.world.features.tree.trunkplacers;
...
public class AspenTrunkPlacer extends TrunkPlacer {
	...
	protected TrunkPlacerType<?> type() {
		return PioneerTreePlacers.Trunk.ASPEN_TRUNK_PLACER.get();
	}
						...
						if(dir.getAxis().isHorizontal()) {
							...
							for(int j = 0; j < length; j++) {
								if (TreeFeature.validTreePos(world, branchPos.offset(dir.getNormal()))) {
									...
									placer.accept(branchPos, PioneerBlocks.ASPEN_LOG.get().defaultBlockState().setValue(RotatedPillarBlock.AXIS, dir.getAxis()));
								}
